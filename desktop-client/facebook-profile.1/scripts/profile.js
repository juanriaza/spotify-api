        ...
        if ('artist' !== obj.listtype || isFacebookProfile) {
          ...
          dom.query('.toplist-share', tl.toplist).forEach(function(elem, index, source) {
            r.fromDOMEvent(elem, 'click').subscribe(function(e) {
              e.preventDefault();
              e.stopPropagation();
              popover.sharePopup(tl.opts.user, elem.parentNode.parentNode.href, elem.parentNode, {
...
models.application.observe(models.EVENT.ARGUMENTSCHANGED, loadUser);
models.session.observe(models.EVENT.STATECHANGED, loadUser);
