...
var AnimatedBanner = {
  ...
  setupBanners: function(testVersion) {
    var self = AnimatedBanner,
        currentOfferId = Math.floor((Math.random() * offers.length)),
        currentOffer = offers[currentOfferId],
        numberOfOffers = Object.keys(offers).length,
        bannerActive = false,
        carouselSpeed = bannerconf.setup.carouselSpeed;

    var header = dom.queryOne('#appsBanner');
    header.innerHTML = '<h2>' + self.headingsLoc.FeaturedSpotifyApps + '</h2>';
    header.innerHTML += fs.readFile('assets/templates/apps-feature-banner.html');
    ...
    header.addEventListener('click', function(e) {
      ...
      while (target.dataset && !target.dataset.appid) {
  ...
  loadBanner: function(header, offer) {
    ...
    var bannerFeatureTemplate = fs.readFile('assets/templates/node-feature.html');
    ...
    var appInfo = [
      icon,
      name,
      (buttonText ? buttonText : AnimatedBanner.headingsLoc.View),
      description
    ];
