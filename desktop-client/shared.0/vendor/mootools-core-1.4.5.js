...
web build:
 - http://mootools.net/core/8218bd87e116d31195ee7304a0bdd2fd

packager build:
 - packager build Core/Core Core/Element.Event Core/Element.Delegation Core/Element.Dimensions Core/Fx Core/Fx.CSS Core/Fx.Tween Core/Fx.Morph Core/Fx.Transitions Core/Request Core/DOMReady
...
}},condition:function(){if(this==i){g();delete Element.Events.load;}return true;}};i.addEvent("load",function(){f=true;});})(window,document);(function(){var a=function(){},b=("XDomainRequest" in window);Request.CORS=new Class({Extends:Request,headers:{},options:{data:null,method:"get",headers:{}},initialize:function(c){this.parent(c);if(b){this.xhr=new XDomainRequest()}},onLoaded:function(){var c=this.xhr;this.running=false;this.status=0;c.onload=a;clearTimeout(this.timer);this.response={text:this.xhr.responseText||"",xml:this.xhr.responseXML};this.success(this.response.text,this.response.xml)},processScripts:function(c){if(this.options.evalResponse){return Browser.exec(c)}return c.stripScripts(this.options.evalScripts)},send:function(m){if(!this.check(m)){return this}this.options.isSuccess=this.options.isSuccess||this.isSuccess;this.running=true;var j=typeOf(m);if(j=="string"||j=="element"){m={data:m}}var f=this.options;m=Object.append({data:f.data,url:f.url,method:f.method},m);var h=m.data,d=String(m.url),c=m.method.toLowerCase();switch(typeOf(h)){case"element":h=document.id(h).toQueryString();break;case"object":case"hash":h=Object.toQueryString(h)}if(this.options.format){var k="format="+this.options.format;h=(h)?k+"&"+h:k}if(this.options.emulation&&!["get","post"].contains(c)){var i="_method="+c;h=(h)?i+"&"+h:i;c="post"}if(this.options.urlEncoded&&["post","put"].contains(c)){var e=(this.options.encoding)?"; charset="+this.options.encoding:"";this.headers["Content-type"]="application/x-www-form-urlencoded"+e}if(!d){d=document.location.pathname}var g=d.lastIndexOf("/");if(g>-1&&(g=d.indexOf("#"))>-1){d=d.substr(0,g)}if(this.options.noCache){d+=(d.contains("?")?"&":"?")+String.uniqueID()}if(h&&c=="get"){d+=(d.contains("?")?"&":"?")+h;h=null}var l=this.xhr;l.open(c.toUpperCase(),d);if(b){l.onload=this.onLoaded.bind(this);l.onerror=this.failure.bind(this)}else{l.onreadystatechange=this.onStateChange.bind(this)}this.fireEvent("request");l.send(h);if(!this.options.async){this.onStateChange()}else{if(this.options.timeout){this.timer=this.timeout.delay(this.options.timeout,this)}}return this}})})();
