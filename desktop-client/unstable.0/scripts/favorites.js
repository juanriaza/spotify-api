/**
 * @fileoverview Functionality for working with favorites.
 */
...
function addFavoriteUser(uri) {
  if (!isFavoriteUser(uri)) {
    ...
    models.User.fromURI(uri, function(user) {
      if (user.data.canonicalUsername) {
        social.subscribe([user.data.canonicalUsername], function() {});
      }
    });
    return true;
  }
...
/**
 * Tests whether a user is a favorite user.
 * @param {string} uri The URI of the user to test.
 * @return {boolean} Whether the specified user is in the favorites.
 */
